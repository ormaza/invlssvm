...
for j =1:numel(gams)
    ...
    % Aloca espaço pra matriz C
    C = zeros(nb_data+1, nb_data+1); 
    saveA = A;
    for l = 1:nb_blocks,
        for c = 1:nb_blocks,
            A = saveA;
            ...
            block_swap = dim*(l-1);
            for pos = 1:dim,
                tmp = A(block_swap+pos,:);
                A(block_swap+pos,:) = A(pos,:);
                A(pos,:) = tmp;
            end
            % troca de colunas
            block_swap = dim*(c-1);
            ...
            % BRI
                  BRI_in = [nb_blocks dim];
                  invA = BRIA(A,BRI_in);
            % Aponta pra linha e coluna aonde deve ser encaixada a
            % submatriz em C
            pl = dim*(l-1)+1:dim*(l-1)+dim;
            pc = dim*(c-1)+1:dim*(c-1)+dim;
            C(pl,pc) = invA(1:dim,1:dim);
            disp(C); pause;
        end
    end
    disp(pinv(saveA)); pause;
